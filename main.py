...
app.layout = html.Div([
    ...
    html.Div([
        ...
        html.Div([
            html.H4(children=['Initial Speed v0 [m/s]']),
...
def display_graph(time, load, v0, ubias, set_point_times, set_point_values):
    set_points = dict(zip(set_point_times, set_point_values))

    ts, step, v_res, error_res, int_res, sp_res = simulate(time, load, v0, ubias, set_points)
    ...
    # Third subplot
    fig.add_trace(go.Scatter(x=ts, y=error_res, mode='lines', name='Error (SP-PV)', line=dict(color='blue', width=3)),
                  row=2, col=1)
